...
import { ArrowLeft, ArrowRight, BookOpen, Check, Flame, Home as HomeIcon, Palette, Play, Plus, RotateCcw, Sparkles, Star, X } from "lucide-react";
import { FREE_PALETTE, PATTERNS, Pattern, targetCount } from "./patterns";
...
import { DELETE_PENDING_KEY, DELETE_TOMBSTONE, emptySaveSnapshot, FREE_DRAWING_LIMIT, FreeDrawing, GameSave, LEGACY_CLEAN_KEY, LEGACY_CLEAN_VALUE, LEGACY_SAVE_KEYS, normalizeSave, readLocalSave, SAVE_KEY, SaveSnapshot, serializeSave, StageEffectId, StageSceneId, StageSelection, StorySelection } from "./save-store";
...
const firstOpenZone = (target: string[], board: string[], color: string, from = 0) =>
  Array.from({ length: 9 }, (_, offset) => (from + offset) % 9)
    .find(zone => zoneIndices(zone).some(index => target[index] === color && board[index] !== color)) ?? 0;
...
function StagePreview({ pattern, selection, compact = false }: { pattern: Pattern; selection: StageSelection; compact?: boolean }) {
  ...
  return <div className={`stage-preview${compact ? " compact" : ""}`} data-scene={selection.scene} data-effect={selection.effect} role="img" aria-label={`${pattern.name}在${scene.name}，${effect.name}`}>
    ...
    <div className="stage-character"><Art pattern={pattern} bead animated /></div>
  </div>;
}
...
function HomeTile({ pattern, colorwayId, onOpen, finished, placed }: { pattern: Pattern; colorwayId?: string; onOpen: () => void; finished: boolean; placed?: number }) {
  ...
  return <button className="home-tile" onClick={onOpen}>
    ...
    <b>{pattern.name}</b>
    <small>{inProgress ? `已拼 ${placed} 颗` : pattern.category === "书桌" ? `${total} 颗 · 拼完能摆` : `${total} 颗 · ${presentation.difficultyLabel}`}</small>
  </button>;
}
...
export default function Home() {
  ...
  const basePattern = PATTERNS.find(p => p.id === activeId) ?? PATTERNS[0];
  ...
  const animationBasePattern = PATTERNS.find(p => p.id === animationId);
  ...
  useEffect(() => {
    if (!celebrate) return;
    const delay = window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 400 : 1800;
  ...
  }, [celebrate, pattern.id]);

  useEffect(() => () => {
    if (messageTimer.current !== null) window.clearTimeout(messageTimer.current);
  }, []);

  const persistSaveNow = async (nextCompleted: string[], nextBoards: Record<string, string[]>, nextActivityDates: string[], nextStages = stagesRef.current, nextColorways = colorwaysRef.current, nextStories = storiesRef.current, nextDrawings = drawingsRef.current) => {
    if (savePhaseRef.current !== "ready") throw new Error("save store is not writable");
    const generation = saveGenerationRef.current;
    const snapshot: SaveSnapshot = { completed: nextCompleted, boards: nextBoards, activityDates: nextActivityDates, stages: nextStages, colorways: nextColorways, stories: nextStories, drawings: nextDrawings };
  ...
  const openGame = (id: string, nextMode?: PlayMode) => {
    ...
    setZone(firstOpenZone(nextTarget, safeBoard, startColor)); setCelebrate(false); setTab("game"); window.scrollTo(0, 0);
  ...
  const enterPlayMode = (nextMode: PlayMode) => {
    ...
    setZone(firstOpenZone(target, boardRef.current, startColor));
  ...
  const openReplay = (id: string, nextMode: "mystery" | "spot") => {
    ...
    if (nextMode === "spot") {
      ...
      if (!puzzle) { replayRef.current = false; setReplay(false); say("这幅还没有另一套配色，先去小舞台看看。"); return; }
    ...
    setSelected(startColor);
    setZone(firstOpenZone(nextTarget, empty, startColor));
    setHint(false);
    setMode("mystery");
    setTab("game");
    window.scrollTo(0, 0);
  ...
  const saveBoardChange = async (next: string[], failure: string) => {
    ...
    if (nextDone === total) {
      ...
      if (!nextStories[pattern.id]) nextStories = { ...nextStories, [pattern.id]: defaultStorySelection(pattern.id) };
    }
    try {
      await persistSaveNow(nextCompleted, nextBoards, nextActivityDates, stagesRef.current, colorwaysRef.current, nextStories);
    } catch {
      boardRef.current = canonicalSaveRef.current.boards[pattern.id] ?? board;
      say(failure, 1600);
      return false;
    }
    setBoard(next); setSavedBoards(nextBoards); setCompleted(nextCompleted); setActivityDates(nextActivityDates); setStories(nextStories);
    if (nextDone === total) { setCelebrate(true); navigator.vibrate?.([40,40,80]); }
    return true;
  };
  ...
  const guideAfterPlacement = (previous: string[], next: string[], color: string) => {
    ...
    if (colorDone && !colorWasDone) {
      ...
      if (mode === "assistant") setZone(firstOpenZone(target, next, nextColor));
    ...
    if (mode === "assistant") {
      const nextZone = firstOpenZone(target, next, color, (zone + 1) % 9);
      setZone(nextZone);
  ...
  const showPoster = async (kind: "print" | "work", posterPattern = pattern) => {
    ...
    const original = PATTERNS.find(candidate => candidate.id === posterPattern.id) ?? posterPattern;
  ...
  return <main>
    <div className="app app-shell" inert={overlayOpen ? true : undefined} aria-hidden={overlayOpen ? true : undefined}>
      ...
      {savePhase === "ready" && tab === "home" && <>
        <button className="home-story" onClick={()=>setStoryPage(0)}>
          <span className="home-story-art" aria-hidden="true"><Art pattern={resolvedPattern(storybookPattern(STORYBOOK_PAGES[0].patternId), colorways[STORYBOOK_PAGES[0].patternId])} animated/></span>
          <span><small>故事绘本</small><b>{STORYBOOK_TITLE}</b><i>翻一页，拼一个角色</i></span>
          <em>打开读</em>
        </button>
        ...
        <section className="home-catalog">
          ...
          <div className="home-grid">{PATTERNS.filter(p=>p.id!==FEATURED_PATTERN.id).map(p=><HomeTile key={p.id} pattern={p} colorwayId={colorways[p.id]} onOpen={()=>openGame(p.id)} finished={completed.includes(p.id)} placed={placedCount(p, savedBoards[p.id])} />)}</div>
        </section>
      ...
      {savePhase === "ready" && tab === "library" && <section className="library"><div className="page-head"><small>图纸宝库</small><h1>今天拼哪个？</h1></div><div className="filters">{categories.map(c=><button key={c} className={filter===c?"active":""} onClick={()=>setFilter(c)}>{c}</button>)}</div><div className="home-grid">{PATTERNS.filter(p=>filter==="全部"||p.category===filter).map(p=><HomeTile key={p.id} pattern={p} colorwayId={colorways[p.id]} onOpen={()=>openGame(p.id)} finished={completed.includes(p.id)} placed={placedCount(p, savedBoards[p.id])} />)}</div></section>}

      {savePhase === "ready" && tab === "game" && <section className="game-screen" data-mode={mode ?? "pick"}>
        <header className="game-header"><button onClick={()=>replay ? leaveReplay() : setTab(originTab)} aria-label={replay ? "返回作品册" : originTab === "home" ? "返回首页" : originTab === "works" ? "返回作品册" : "返回图纸宝库"}><ArrowLeft aria-hidden="true"/></button><div><b>{pattern.name}</b><small>{mode === "spot" && spotPuzzle ? `找到 ${spotFound.length}/${spotPuzzle.swapped.length} 颗 · ${spotPuzzle.homeName}` : `${done}/${total} 颗${mode && mode !== "assistant" && mode !== "spot" ? ` · 错误 ${mistakes}` : ""}${activeColorway ? ` · ${activeColorway.name}` : ""}`}</small></div><div className="game-header-actions">{mode === "mobile" && <button type="button" onClick={()=>setTapMode(v=>!v)}>{tapMode ? "点击模式" : "拖动模式"}</button>}{mode && mode !== "assistant" && mode !== "spot" ? <button onClick={()=>setHint(v=>!v)}>{hint?"关提示":"开提示"}</button> : <span />}</div></header>
      ...
      {savePhase === "ready" && tab === "works" && <section className="works">
        ...
        {completed.length ? <div className="work-grid">{PATTERNS.filter(p=>completed.includes(p.id)).map(p=>{const selection=stages[p.id]??DEFAULT_STAGE;const scene=STAGE_SCENES.find(item=>item.id===selection.scene)??STAGE_SCENES[0];const colorway=selectedColorway(p,colorways[p.id]);const workPattern=resolvedPattern(p,colorway.id);const story=composeStory(p.id,stories[p.id],selection.scene,selection.effect);return <article key={p.id}><button onClick={()=>openPatternStage(p.id)}><StagePreview pattern={workPattern} selection={selection} compact/><b>{p.name}</b><small>{colorway.name} · {scene.name}</small><p className="work-story">{story.line}</p></button><div className="work-actions"><button className="work-play" onClick={()=>openPatternStage(p.id)}><Play aria-hidden="true"/>进入小舞台</button><button className="work-play secondary" onClick={()=>openReplay(p.id,"spot")}>配色找不同</button></div></article>})}</div> : savedDrawings.length ? null : <div className="no-works"><span><Sparkles aria-hidden="true"/></span><h2>第一颗星星还在等你</h2><p>完成一张图纸，它就会出现在这里。</p><button onClick={()=>openGame(FEATURED_PATTERN.id)}>去完成第一幅</button></div>}
    ...
    {bookPage && bookCast && bookArt && <DialogFrame className="story-sheet" label={STORYBOOK_TITLE} onClose={()=>setStoryPage(null)}>
        <section>
          ...
          <p>{bookPage.text}</p>
          <button className="story-craft" onClick={()=>{ const id = bookCast.id; setStoryPage(null); openGame(id); }}>拼出{bookCast.name}</button>
          <div className="story-nav">
            <button disabled={storyPage === 0} onClick={()=>setStoryPage((storyPage ?? 0) - 1)}>上一页</button>
            <button disabled={storyPage === STORYBOOK_PAGES.length - 1} onClick={()=>setStoryPage((storyPage ?? 0) + 1)}>{storyPage === STORYBOOK_PAGES.length - 1 ? "读完了" : "下一页"}</button>
          </div>
        </section>
      </DialogFrame>}
